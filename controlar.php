...
if (isset($_POST['boton_controlar'])) { 
  ...
  if($query4 = $connect->query($sql1)){
    ...
    $message.=' <tr>
          ...
          $message.='</table>'; //se controla y se envia a Presidencia stellam.procaccini@daxa.com.ar
  ...
  list($nombre1, $apellido1) = explode(" ", $presentado);
  $sql_correo3 = "SELECT Email FROM usuarios WHERE Nombre LIKE '%$nombre1%' AND Apellido LIKE '%$apellido1%'";
  $query_sql = $connect->query($sql_correo3);
  $result_sql3 = $query_sql->fetch_array();

  $sql_lugar4 = "SELECT lugar FROM `comprobante` WHERE Nro_comprobante = $nrocomprobante";
  $query_sql4 = $connect->query($sql_lugar4);
  $result_sql4 = $query_sql4->fetch_array();

  $correo = $result_sql3['Email']; //email de la persona que preparó la solicitud
  $lugar =  $result_sql4['lugar'];
  ...
  if($query5 = $connect->query($sql2)){
   ...
   <td style="background-color: #ecf0f1">
      <div style="color: #34495e; margin: 4% 10% 2%; text-align: justify;font-family: sans-serif">
       ...
       <p style="margin: 2px; font-size: 15px">
         Se le informa que su solicitud de caja ha sido rechazada por '.$rechazo.'. Ingrese al sistema para solicitar un nuevo pedido o comuníquese con su gerente del sector. <br><br><br>
      ...
      <tr>
       ...
       <br>
        ...
        <p style="color: #b3b3b3; font-size: 12px; text-align: center;margin: 30px 0 0">Daxa Argentina 2022
          ...
          $message.='</table>'; //se rechaza y se envía una notificación a la persona que preparó
    $enviado = $mailSend-> Enviar("Comprobantes |DAXA|",$presentado,$correo,"Rechazo de solicitud", $message);
      if($enviado){
          echo "<meta http-equiv='refresh' content='0'>";
      }else {
          echo ("Error al enviar el correo, por favor contacte con su administrador");
...
<body class="w3-light-grey">
  ...
  <!-- Efecto overlay para pantallas pequeñas -->
      ...
      <form action="<?php $_SERVER['PHP_SELF'] ?>" method="POST">
        ...
        <div class="form-row">
          <div class="form-group col-md-6">
            ...
            <input type="text" style="font-weight:bold;" class="form-control" id="nrocomprobante" name="nrocomprobante" required>
          </div>
          <div class="form-group col-md-6">
        ...
        <div class="form-row">
          <div class="form-group col-md-6">
            <label for="inputcon"><b>Controló/Rechazó</b></label>
            <input type="text" style="font-weight:bold;" class="form-control" id="controlo" value="<?php echo $result['Nombre'] . " " . $result['Apellido'] ?>" name="controlo" required>
          </div>
          <div class="form-group col-md-3">
            <label for="inputfec"><b>Fecha</b></label>
            <input type="text" style="font-weight:bold;" class="form-control" id="fecha" name="fecha" required>
          </div>
          <div class="form-group col-md-3">
            ...
            <div class="input-group">
              ...
              <input type="text" style="font-weight:bold;" class="form-control" id="total" name="total" required>
            </div>
          </div>
        </div>
        <div class="form-group">
          <label for="inputMail"><b>Detalles</b></label>
          <input type="text" class="form-control" id="detalles" placeholder="Detalles de la operación / Motivo del rechazo" aria-describedby="emailHelp" name="detalles" required>
        </div>
